...
public class StreamCypher {
    ...
    public static byte[] transformByteArray(byte[] array, String startLFSR) {
        boolean[] startKey = stringToBoolArr(startLFSR);
        ...
        for(int i = 0; i < array.length; i++) {
            ...
            System.out.println("Byte " + (i + 1) + " / " + array.length);
        }
        return array;
    }

    public static String transformForLog(byte[] array, String startLFSR) {
        boolean[] startKey = stringToBoolArr(startLFSR);
        if(startKey == null) return null;
        LFSR lfsr = new LFSR(startKey);
